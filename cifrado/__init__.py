...
def exists():
    """cifrado.c existe"""
    check50.exists("cifrado.c")


@check50.check(exists)
def compiles():
    """cifrado.c compila"""
    check50.c.compile("cifrado.c", lcs50=True)


@check50.check(compiles)
def encrypt1():
    """encripta de la "A" a la "Z" usando ZYXWVUTSRQPONMLKJIHGFEDCBA como clave"""
    check50.run("./cifrado ZYXWVUTSRQPONMLKJIHGFEDCBA").stdin("A").stdout("texto encriptado:\s*Z\n", "texto encriptado: Z\n").exit(0)
...
def encrypt3():
    ...
    check50.run("./cifrado NJQSUYBRXMOPFTHZVAWCGILKED").stdin("ABC").stdout("texto encriptado:\s*NJQ\n", "texto encriptado: NJQ\n").exit(0)


@check50.check(compiles)
def encrypt4():
    """encripta "XyZ" como "KeD" usando NJQSUYBRXMOPFTHZVAWCGILKED como clave"""
    check50.run("./cifrado NJQSUYBRXMOPFTHZVAWCGILKED").stdin("XyZ").stdout("texto encriptado:\s*KeD\n", "texto encriptado: KeD\n").exit(0)


@check50.check(compiles)
def encrypt5():
    """encripta "This is CS50" como "Cbah ah KH50" usando YUKFRNLBAVMWZTEOGXHCIPJSQD as clave"""
    check50.run("./cifrado YUKFRNLBAVMWZTEOGXHCIPJSQD").stdin("This is CS50").stdout("texto encriptado:\s*Cbah ah KH50\n", "texto encriptado: Cbah ah KH50\n").exit(0)
...
def handles_multiple_duplicate_chars():
    ...
    check50.run("./cifrado MMCcEFGHIJKLMNOPqRqTUVWXeZ").exit(1)
